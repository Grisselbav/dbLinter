...
| APEXlang  | `apexlang` | [Oracle SQL Developer Extension for VSCode](https://marketplace.visualstudio.com/items?itemName=Oracle.sql-developer) | `.apx` |
...
Only [fenced code blocks](https://github.github.com/gfm/#fenced-code-blocks) using ```` ``` ```` are checked for Markdown and APEXlang. Code blocks marked for languages other than `sql` or  `plsql` are ignored. Code blocks without a language specifier are considered.
