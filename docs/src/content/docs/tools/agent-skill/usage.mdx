...
| "Only check new code compared to main"                         | `dblinter check --outputFormats=sarif --newCodeOnly=true --referenceBranch=main $(git diff --name-only --diff-filter=ACMR main)` — `--newCodeOnly` alone doesn't limit which files are scanned, so the agent also passes the changed files explicitly. Note: `main` (the local branch) and `origin/main` (the remote-tracking ref) can differ — the agent uses whichever one you actually meant, not a silent default |
