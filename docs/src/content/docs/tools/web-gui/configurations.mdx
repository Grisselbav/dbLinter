...
Furthermore, the database connection can be used optionally in

- Checks, e.g. in [G-7120: Always add the name of the program unit to its end keyword](https://dblinter.app/ords/r/dblinter/dblinter-console/rules#P1000_SHOW_RULE=core%20g-7120)
to determine if the database version lets you add end labels to a `create type body` statement.
- Quick fixes, e.g. to determine the default format model in [G-9010: Always use a format model in string to date/time conversion functions](https://dblinter.app/ords/r/dblinter/dblinter-console/rules#P1000_SHOW_RULE=core%20g-9010).
...
In fact, you can configure all the attributes needed to establish a database connection directly in the client.

To check the code in the [dbLinter Repo](/dbLinter/reference/architecture/#repo), we enabled all rules.
One of these is [G-1110: Avoid connect users that own database objects](https://dblinter.app/ords/r/dblinter/dblinter-console/rules#P1000_SHOW_RULE=core%20g-1110).
This rule is implemented as an SQL-based test, meaning a query must be run against the Data Dictionary to determine if connect users own any database objects.
...
With a [Starter Subscription (Free)](/dbLinter/getting-started/starter-subscription/), you can only have one configuration.
...
Press the wrench icon in the configure column to see the details of a configuration.
