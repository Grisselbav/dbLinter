...
The CLI is included in the VS Code extension, but it can also be used standalone,
...
The dbLinter VS Code Extension can be installed in any IDE that implement the [VS Code extension API](https://code.visualstudio.com/api),
...
The extension provides the rule violations for URIs in scope via the included language server.
