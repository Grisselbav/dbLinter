...
- [authentication](/dbLinter/tools/web-gui/security/#authentication)
- [authorisation](/dbLinter/tools/web-gui/security/#authorisation)
...
However, we are considering providing a read-only client-side REST API for air-gapped environments.
This API would serve requests from a local, read-only data store which could be periodically refreshed with a dedicated export file.
This means that all developers and CI/CD pipelines could use dbLinter even if they don't have internet access.

Nevertheless, in order to manage configurations and custom rules, as well as to export the latest configurations, you will need to use the dbLinter [Web GUI](/dbLinter/tools/web-gui/web-gui-overview/).
